...
async def test_patients_pagination_total_matches_full(client: AsyncClient):
    ...
    paginated = await client.post(
        "/api/patients", json={"page": 1, "pageSize": 20}, headers=_bearer(token),
    )
...
async def test_patients_cursor_walk_covers_all_without_duplicates(client: AsyncClient):
    ...
    while r.json()["next"] is not None:
        r = await client.post(
            "/api/patients", json={"cursor": r.json()["next"]}, headers=_bearer(token),
        )
        assert r.status_code == 200
...
async def test_patients_last_page_has_null_next_and_beyond_is_empty(client: AsyncClient):
    ...
    first = await client.post(
        "/api/patients", json={"page": 1, "pageSize": 50}, headers=_bearer(token),
    )
    last_page = first.json()["totalPages"]
    r = await client.post(
        "/api/patients", json={"page": last_page, "pageSize": 50}, headers=_bearer(token),
    )
    assert r.json()["next"] is None
    r = await client.post(
        "/api/patients", json={"page": last_page + 1, "pageSize": 50}, headers=_bearer(token),
    )
    assert r.status_code == 200
...
async def test_patients_page_size_is_clamped_to_100(client: AsyncClient):
    token = await _get_access_token(client)
    r = await client.post(
        "/api/patients", json={"page": 1, "pageSize": 500}, headers=_bearer(token),
    )
    assert r.status_code == 200
...
async def test_patients_malformed_cursor_422_without_echo(client: AsyncClient):
    token = await _get_access_token(client)
    r = await client.post(
        "/api/patients", json={"cursor": "not-a-cursor-xyz"}, headers=_bearer(token),
    )
...
async def test_patients_invalid_body_422_without_echo(client: AsyncClient):
    token = await _get_access_token(client)
    r = await client.post(
        "/api/patients", json={"page": "zero-ish", "pageSize": -1}, headers=_bearer(token),
    )
    assert r.status_code == 422
...
async def test_patients_user_mode_rejects_service_token(
    client: AsyncClient, service_issuer: ServiceTokenIssuer,
):
    r = await client.post(
        "/api/patients", json={"employeeId": "12345"}, headers=_bearer(service_issuer.mint()),
    )
    assert r.status_code == 401
...
async def service_client(service_issuer: ServiceTokenIssuer) -> AsyncIterator[AsyncClient]:
    ...
    state.store.users.save(User(
        id="user-099", username="park@hospital.com", password="password",
        name="박의사", employee_id="99999", department="내과",
        email="park@hospital.com", email_verified=True, active=False,
    ))
...
async def test_service_patients_by_employee_id(
    service_client: AsyncClient, service_issuer: ServiceTokenIssuer,
):
    r = await service_client.post(
        "/api/patients", json={"employeeId": "12346", "page": 1, "pageSize": 10},
...
async def test_service_patients_cursor_carries_employee_id(
    ...
    headers = _bearer(service_issuer.mint())
    r = await service_client.post(
        "/api/patients", json={"employeeId": "12346", "page": 1, "pageSize": 10}, headers=headers,
    )
    first_uids = [p["patientUid"] for p in r.json()["patients"]]
    r2 = await service_client.post(
        "/api/patients", json={"cursor": r.json()["next"]}, headers=headers,
    )
...
async def test_service_patients_requires_employee_id_or_cursor(
    ...
    r = await service_client.post(
        "/api/patients", json={"page": 1, "pageSize": 10}, headers=_bearer(service_issuer.mint()),
    )
    assert r.status_code == 422
...
async def test_service_patients_unknown_or_inactive_is_empty_200(
    service_client: AsyncClient, service_issuer: ServiceTokenIssuer, employee_id: str,
):
    """담당 환자 0명인 재직 직원과 같은 모양 — 이 엔드포인트로 직원 존재를 알 수 없어야 한다."""
    r = await service_client.post(
        "/api/patients", json={"employeeId": employee_id, "page": 1, "pageSize": 10},
        headers=_bearer(service_issuer.mint()),
    )
    assert r.status_code == 200
    assert r.json() == {
        "patients": [], "page": 1, "pageSize": 10, "totalCount": 0, "totalPages": 0, "next": None,
    }
...
async def test_service_patients_requires_patients_read_scope(
    ...
    r = await service_client.post(
        "/api/patients", json={"employeeId": "12346"},
...
async def test_employee_exact_match_returns_200(
    ...
    r = await service_client.post(
        "/api/employee", json={"employeeId": "12345", "name": "이의사"},
        headers=_bearer(service_issuer.mint()),
    )
    assert r.status_code == 200
    assert r.json() == {
        "employeeId": "12345", "name": "이의사", "department": "내과", "active": True,
    }
    _assert_no_store(r.headers)
...
async def test_employee_trims_surrounding_whitespace_only(
    ...
    headers = _bearer(service_issuer.mint())
    r = await service_client.post(
        "/api/employee", json={"employeeId": " 12345 ", "name": "  이의사 "}, headers=headers,
    )
    assert r.status_code == 200
    r = await service_client.post(
        "/api/employee", json={"employeeId": "12345", "name": "이 의사"}, headers=headers,
    )
    assert r.status_code == 404
...
async def test_employee_miss_is_indistinguishable_404(
    service_client: AsyncClient, service_issuer: ServiceTokenIssuer, body: dict[str, str],
):
    r = await service_client.post("/api/employee", json=body, headers=_bearer(service_issuer.mint()))
    assert r.status_code == 404
    assert r.content == b""
    _assert_no_store(r.headers)
...
async def test_employee_validation_error_422_without_echo(
    ...
    r = await service_client.post(
        "/api/employee", json={"employeeId": "12345-echo-me"}, headers=_bearer(service_issuer.mint()),
    )
    assert r.status_code == 422
...
async def test_employee_requires_service_token(service_client: AsyncClient):
    ...
    r = await service_client.post(
        "/api/employee", json={"employeeId": "12345", "name": "이의사"}, headers=_bearer(user_token),
    )
    assert r.status_code == 401
