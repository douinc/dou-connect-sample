...
| `records.transcribed` | 서버 전사(STT) 완료 — 전사가 만들어진 기록에만 발송 |
| `records.updated` | 요약 편집·재생성 — 수신 시 기록을 **다시 조회**해 로컬 사본 갱신 |
| `records.deleted` | 기록 삭제·연결 해제 — 수신 후 단건 조회가 404를 반환하는 것이 **정상** |
...
샘플 코드는 [MIT 라이선스](LICENSE) 아래 배포되며 자유롭게 사내 프로젝트에 복사·수정해 사용하실 수 있습니다. **단, 본 샘플은 어떤 보증도 없이 "있는 그대로(as is)" 제공되며, 보안·영속성·운영 적합성에 대한 책임은 사용 측에 있습니다** (LICENSE 참조). 연동 관련 문의: [team@dou.so](mailto:team@dou.so).
