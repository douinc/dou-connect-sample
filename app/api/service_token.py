...
class ServiceTokenVerifier:
    def __init__(
        self, *, issuer: str, audience: str, key_source: SigningKeySource,
    ) -> None:
        self._issuer = issuer
    ...
    @classmethod
    def from_jwks_url(
        cls, *, jwks_url: str, issuer: str, audience: str,
    ) -> ServiceTokenVerifier:
        # JWKS는 Cache-Control max-age=3600 — 1시간 캐시, 모르는 kid면 PyJWKClient가
        # 즉시 재조회한다 (키 교체 직후 대응). 가이드의 캐싱 전략 그대로.
        return cls(
            issuer=issuer, audience=audience,
            key_source=jwt.PyJWKClient(jwks_url, cache_keys=True, lifespan=3600),
        )
    ...
    def verify(self, token: str, *, required_scope: str) -> dict[str, Any]:
        ...
        if not isinstance(scopes, list) or required_scope not in scopes:
            raise ServiceTokenError(
                403, "insufficient_scope", "required scope missing", scope=required_scope,
            )
        return claims


def require_service_token(
    *, authorization: str | None, verifier: ServiceTokenVerifier, required_scope: str,
) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=exc.status, detail=exc.error,
