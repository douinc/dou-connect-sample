...
_NO_ECHO_PATHS = frozenset({"/api/patients", "/api/employee"})
...
def install_exception_handlers(app: FastAPI) -> None:
    ...
    async def _no_echo_422(request: Request, exc: RequestValidationError) -> Response:
        if request.url.path in _NO_ECHO_PATHS:
            return JSONResponse(
...
def build_api_router(state: AppState) -> APIRouter:
    ...
    def _service_claims(authorization: str | None, scope: str) -> dict[str, Any]:
        ...
        claims = require_service_token(
            authorization=authorization, verifier=state.service_verifier, required_scope=scope,
        )
    ...
    async def patients(
        body: PatientsQuery, authorization: str | None = Header(default=None),
    ) -> Response:
        ...
        if state.settings.patients_auth_mode == "user":
            ...
            if cursor is not None and cursor.employee_id != user.employee_id:
    ...
    @router.post("/employee", response_model=EmployeeInfo, response_model_by_alias=True,
                 response_model_exclude_none=True,
                 responses={404: {"description": "일치하는 재직 직원 없음 — 본문 없음. "
                                                 "미존재/이름 불일치/퇴직을 구분하지 않는다."}})
    async def employee(
        body: EmployeeQuery, response: Response,
        ...
        사번·이름을 남기지 않는다.
        """
        _service_claims(authorization, SCOPE_STAFF_READ)
        ...
        response.headers.update(NO_STORE_HEADERS)
        return EmployeeInfo(
            employeeId=user.employee_id, name=user.name.strip(), department=user.department,
        )
...
def _patients_response(
    dtos: list[PatientDTO], body: PatientsQuery, cursor: Cursor | None, employee_id: str,
) -> JSONResponse:
    if cursor is None and body.page is None and body.page_size is None:
        ...
        resp = PatientListResponse(
            patients=page.items, page=page.page, pageSize=page.page_size,
            totalCount=page.total_count, totalPages=page.total_pages, next=nxt,
        )
